  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, snapshot => {
      ...
      const availabilityList = Object.entries(vehicleTypes).map(([plate, vehicle]) => {
        const assignedTrip = trips.find(trip => trip.plateNumber === plate && trip.dateTime?.slice(0,10) === todayStr && trip.status !== "Fulfilled");
        return { plateNumber: plate, description: vehicle.description, status: assignedTrip ? `Assigned (${assignedTrip.tripCode})` : "Available" };
      });
  ...
  return (
    <div className="bg-gradient-to-br from-slate-50 to-slate-100 min-h-screen w-full px-6 py-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
        {bulletins.map(b => (
          <div key={b.id} className="flex flex-col bg-white p-4 rounded-xl shadow hover:shadow-md transition hover:bg-slate-50">
      ...
      <div className="w-full flex gap-4">
        ...
        <div className="w-[20%] flex flex-col gap-4">
          <HomeCard href="/request-form" title="Request Form" icon={<FileTextIcon className="w-8 h-8 text-blue-600"/>} description="Create a new vehicle service request"/>
          <HomeCard href="/requests" title="Requests" icon={<ClipboardClockIcon className="w-8 h-8 text-amber-600"/>} description="View and manage your service requests"/>
          <HomeCard href="/trips" title="Approved Trips" icon={<MapPinCheckIcon className="w-8 h-8 text-green-600"/>} description="Track your approved vehicle trips"/>

          {/* Maintenance Dropdown */}
          <div className="relative">
            <div onClick={() => setShowMaintenance(!showMaintenance)} className="cursor-pointer bg-white hover:bg-slate-50 border border-slate-200 p-6 rounded-xl shadow-sm hover:shadow-md transition">
              <div className="flex flex-col items-center text-center space-y-3">
                ...
                <p className="text-slate-600 text-center">Track and Manage Vehicle Maintenance Tasks</p>
              </div>
            </div>
            {showMaintenance && (
              <div className="absolute top-full left-0 mt-2 w-full bg-white border border-slate-200 rounded-xl shadow-lg p-3 flex flex-col space-y-1 z-50">
                <Link href="/maintenance/automotive"><DropdownItem icon={<CarIcon className="w-6 h-6 text-teal-600"/>} label="Automotive"/></Link>
                <Link href="/maintenance/motorcycle"><DropdownItem icon={<BikeIcon className="w-6 h-6 text-teal-600"/>} label="Motorcycle/Trimobile"/></Link>
              </div>
            )}
          </div>
        </div>
        ...
        {/* CENTER PANEL */}
        <div className="w-[60%]">
          <VehicleAvailabilityPanel availability={availability} maintenanceReports={maintenanceReports} loading={loadingVehicles} anyAvailable={anyAvailable}/>
        </div>

        {/* RIGHT PANEL */}
        <div className="w-[20%] flex flex-col gap-4">
          <HomeCard href="/analytics" title="Data & Analytics" icon={<ChartColumnIcon className="w-8 h-8 text-purple-600"/>} description="Analyze vehicle usage data"/>
          ...
          {/* Reports Dropdown */}
          <div className="relative">
            <div onClick={() => setShowReports(!showReports)} className="cursor-pointer bg-white hover:bg-slate-50 border border-slate-200 p-6 rounded-xl shadow-sm hover:shadow-md transition">
              <div className="flex flex-col items-center text-center space-y-3">
            ...
            {showReports && (
              <div className="absolute top-full left-0 mt-2 w-full bg-white border border-slate-200 rounded-xl shadow-lg p-3 flex flex-col space-y-1 z-50">
                <Link href="/fuel-reports"><DropdownItem icon={<DropletIcon className="w-6 h-6 text-indigo-600"/>} label="Fuel Reports"/></Link>
                <Link href="/maintenance-reports"><DropdownItem icon={<WrenchIcon className="w-6 h-6 text-indigo-600"/>} label="Maintenance Reports"/></Link>
              </div>
            )}
          </div>
        </div>
...
function DropdownItem({ icon, label }: { icon: ReactNode; label: string }) {
  return <div className="flex items-center gap-3 p-2 rounded-lg hover:bg-slate-50 cursor-pointer">{icon}<span className="text-slate-700 font-medium">{label}</span></div>;
}

function VehicleAvailabilityPanel({ availability, maintenanceReports, loading, anyAvailable }: { availability: VehicleAvailabilityItem[], maintenanceReports: MaintenanceReport[], loading: boolean, anyAvailable: boolean }) {
  return (
    <div className="w-full bg-white p-6 rounded-xl shadow flex flex-col items-center">
      ...
      {loading ? <p>Loading...</p> :
        <div className="grid grid-cols-5 gap-4 w-full">
          {availability.map(v => {
            ...
            if (defectiveReport) { statusText = "Under Maintenance"; statusColor = "text-red-600"; }
            else if (isAssigned) { statusText = v.status; statusColor = "text-yellow-600"; }

            return (
              <div key={v.plateNumber} className="flex flex-col items-center bg-gray-50 p-4 rounded-xl shadow">
                <div className="w-20 h-20 bg-slate-200 rounded-full flex items-center justify-center font-bold text-base">{v.description.slice(0,2)}</div>
      ...
      <Link href={anyAvailable ? "/request-form" : "#"} className="w-full mt-4">
        <button className={`w-full py-3 rounded text-white font-semibold ${anyAvailable ? "bg-blue-500 hover:bg-blue-600" : "bg-gray-400 cursor-not-allowed"}`} disabled={!anyAvailable}>
