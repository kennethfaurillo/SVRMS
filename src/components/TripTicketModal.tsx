...
export default function TripTicketModal({
  ...
  if (!tripData?.requestedVehicle && !tripData?.vehicleAssigned) return;
    ...
    if (!showTripTicket || !requestId) {
      ...
      try {
        ...
        if (!querySnapshot.empty) {
          ...
          console.log("✅ Fetched Trip Data:", data); // Para makita mo sa console kung may ETA
...
const getPrintHtml = () => {
  ...
  const passengersText = tripData.passengers 
   ...
   const eta = tripData.estimatedTimeOfArrival 
     ? tripData.estimatedTimeOfArrival 
     : (tripData.returnHomeTime || "—");
  const speedoReading = tripData.speedometerReading || "—";
  const fuelInTank = tripData.fuelInTank || "—";
  const refuelAmount = tripData.refuelAmount || "0.5L";
      ...
      <body>
        <div class="ticket">
          ...
          <h2>DRIVER'S TRIP TICKET</h2>
            ...
            <tr><th>Fuel in Tank</th><td colspan="3">${fuelInTank} L</td></tr>
            <tr><th>Refuel</th><td colspan="3">${refuelDisplay}</td></tr>
          </table>
...
<table style={{ width: "100%", borderCollapse: "collapse", border: "2.5px solid #000", marginBottom: 30 }}>
          <tbody>
            ...
            {/* Bagong Fields mula sa handwritten form */}
                       <tr>
              <th style={{ border: "1px solid #000", padding: 12, background: "#f0f0f0" }}>Estimated Time of Arrival</th>
              <td style={{ border: "1px solid #000", padding: 12 }}>
                <input
                  type="text"
                  value={tripData.estimatedTimeOfArrival || ""}
                  onChange={(e) => setTripData({ ...tripData, estimatedTimeOfArrival: e.currentTarget.value })}
            ...
            <tr>
              ...
              <td style={{ border: "1px solid #000", padding: 12 }}>
                <input
                  type="text"
                  value={tripData.speedometerReading || ""}
                  onChange={(e) => setTripData({ ...tripData, speedometerReading: e.currentTarget.value })}
                  style={{ width: "100%", border: "none", borderBottom: "2px solid #000" }}
                  placeholder="e.g. 23557 km"
                />
              </td>
            </tr>
            <tr>
            ...
            <tr>
              ...
              <td style={{ border: "1px solid #000", padding: 12 }}>
                <input
                  type="text"
                 value={
                maintenanceData?.driverSection?.balanceInTank !== undefined
                  ? (
                      Number(maintenanceData.driverSection.balanceInTank) <=
                      ((tripData.requestedVehicle || "").includes("TRYC") ? 10 : 20) * 0.2
                    )
                    ? "⚠️ NEED REFUEL"
                    : "NO NEED"
                  : "—"
              }
                  onChange={(e) => setTripData({ ...tripData, refuelAmount: e.currentTarget.value })}
                  style={{ width: "100%", border: "none", borderBottom: "2px solid #000" }}
                />
              </td>
            </tr>
